# SCMS — Local Development Setup

> ← Back to the [project overview](../README.md)

A classroom management platform for a Sri Lankan tutoring institute: student/teacher records, QR-based attendance with AI-assisted CCTV headcount and face verification, payments (bank receipt + PayHere), exams, learning materials, promotions, and WhatsApp notifications — integrated with a Moodle LMS for course content and enrollment sync.

This document reflects the codebase directly (routes, controllers, `.env.example`, `package.json`, `vite.config.js`, `create_tables.js`/`import_database.js`) as of **August 2026**. Older docs (now kept in [`docs/archive/`](archive/)) reference a Docker setup, package versions, and URLs that no longer match the actual code — this file supersedes them for local setup.
...
├── Dockerfile                   # Production image (Render) — backend + AI service in one container
├── deploy/                      # Deployment guides + scripts — see deploy/DEPLOY.md
├── docs/                        # This guide, screenshots, archived older docs
...
**Docker:** the root `Dockerfile` is only used for the production deployment on Render (one container running the Node backend + the Python AI service — see [`deploy/DEPLOY.md`](../deploy/DEPLOY.md)). There is no `docker-compose.yml`; local development is the manual setup below.
...
| `PAYHERE_MERCHANT_ID` / `PAYHERE_MERCHANT_SECRET` / `PAYHERE_MODE` | PayHere sandbox credentials, if testing online payments (`PAYHERE_MODE=sandbox`) |
| `MOODLE_URL` / `MOODLE_TOKEN` | Only needed if running Moodle locally too — see §4.7 |
| `WHATSAPP_SESSION_DIR` | Optional — where the WhatsApp (Baileys) login session is stored for notifications |
...
Only needed if you're testing Moodle-integrated features (course sync, SSO). See [`moodle_setup.md`](../moodle_setup.md) for the full restore procedure — in short: install XAMPP (§2), copy `moodle/` into XAMPP's `htdocs/`, copy `moodledata/` alongside it, and restore the provided `database/moodle.backup` into XAMPP's MySQL. The restored install already has its web-service token configured — put that token in the backend's `MOODLE_TOKEN`.
...
[`deploy/DEPLOY.md`](../deploy/DEPLOY.md) — this project's plan: Neon/Render Postgres + Render backend + Vercel
...
- Switch `PAYHERE_MODE` to `live` (with live merchant credentials) only when actually ready to accept real payments.
